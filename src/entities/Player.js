export class Player {
  ...
  _createAnimations() {
    this.scene.anims.create({
      key: 'idle',
      frames: this.scene.anims.generateFrameNumbers('idle', { start: 0, end: 0 }),
      frameRate: 8,
      repeat: -1
    })
    this.scene.anims.create({
      key: 'walk-down',
      frames: this.scene.anims.generateFrameNumbers('walk', { frames: [0, 4, 8, 12] }),
      frameRate: 8,
      repeat: -1
    })
    this.scene.anims.create({
      key: 'walk-up',
      frames: this.scene.anims.generateFrameNumbers('walk', { frames: [1, 5, 9, 13] }),
      frameRate: 8,
      repeat: -1
    })
    this.scene.anims.create({
      key: 'walk-left',
      frames: this.scene.anims.generateFrameNumbers('walk', { frames: [2, 6, 10, 14] }),
      frameRate: 8,
      repeat: -1
    })
    this.scene.anims.create({
      key: 'walk-right',
      frames: this.scene.anims.generateFrameNumbers('walk', { frames: [3, 7, 11, 15] }),
      frameRate: 8,
      repeat: -1
    })
  }
  ...
  update(speed = 200) {
    ...
    if (this.canDash && hasDirection && this.wasd.space.isDown && this.dashCooldownTimer === 0) {
